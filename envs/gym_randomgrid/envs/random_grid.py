...
import envs.gym_simplegrid.rendering as r
from envs.gym_simplegrid.window import Window
...
class RandomGridEnv(Env):
    ...
    def reset(
        ...
        if options is None:
            options = {'start_loc': (3,0), 'goal_loc': (0,7), 'goal_prob': 0.5}
    ...
    def get_transprob(self, x, y, action):
        # free grid cells
        # if self.is_free(x, y):
        max_prob = 0.8
        other_prob = (1 - max_prob) / 2
        if action == 0:
            probs = [max_prob, 0, other_prob, other_prob]
        if action == 1:
            probs = [0, max_prob, other_prob, other_prob]
        if action == 2:
            probs = [other_prob, other_prob, max_prob, 0]
        if action == 3:
            probs = [other_prob, other_prob, 0, max_prob]
        # make unreachable state available, return opposite action
        # else:
        #     if action == 0:
        #         probs = [0, 0.8, 0.1, 0.1]
        #     if action == 1:
    ...
    def _chooseActionProb(self, x, y, action):
        actions, probs = self.get_transprob(x, y, action)
        return np.random.choice(actions, p=probs)
    
        # if self.is_free(x, y):
        #     if action == 0:
        #         return np.random.choice([0, 2, 3], p=[0.8, 0.1, 0.1])
        #     if action == 1:
        #         return np.random.choice([1, 2, 3], p=[0.8, 0.1, 0.1])
        #     if action == 2:
        #         return np.random.choice([2, 0, 1], p=[0.8, 0.1, 0.1])
        #     if action == 3:
        #         return np.random.choice([3, 0, 1], p=[0.8, 0.1, 0.1])
        # # make unreachable state available, return opposite action
        # else:
        #     if action == 0:
        #         return 1
    ...
    def step(self, action: int):
        ...
        if not self.deterministic:
            action = self._chooseActionProb(row, col, action)
        ...
        return self.get_obs(), self.reward, self.get_done(), False, self.get_info()
    ...
    def compute_rprob(self, x:int, y:int) -> float:

        def tuple_minus_abs(coord1, coord2):
            return [abs(x-y) for x,y in zip(coord1, coord2)]
        
        reward_prob = 0.09*sum(tuple_minus_abs((x,y), self.start_xy))
        
        return reward_prob

    def Bernoulli_reward(self, p: float) -> float:
        """
        Sample a Bernoulli reward with probability p.
        """
    ...
    def get_reward(self, x: int, y: int) -> float:
        ...
        if not self.is_in_bounds(x, y):
            ...
            return self.Bernoulli_reward(self.max_prob)
        else:
            return -0.1
    ...
    def get_info(self) -> dict:
        return {'agent_xy': self.agent_xy, 'x': self.target_row, 'y': self.target_col}
    ...
    def render_frame(self, tile_size=r.TILE_PIXELS, highlight_mask=None):
        ...
        for x in range(self.nrow):
            for y in range(self.ncol):
                if self.obstacles[x,y] == self.GOAL:
                    cell = r.Wall(color='green')
                else:
                    cell = None
