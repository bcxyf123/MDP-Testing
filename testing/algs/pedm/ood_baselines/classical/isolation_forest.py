...
class ISOFOREST_Detector(Base_Detector):
    ...
    def _fit(self, train_ep_data, *args, **kwargs) -> None:
        ...
        X_val = np.concatenate([ep.states for ep in train_ep_data], axis=0)
        self.threshold = self._predict_scores(X_val, None).max(0)
