...
class KNN_Detector(Base_Detector):
    def __init__(self, env, knn_index=None, th=None, k=1, use_acts=True, use_n_obs=True, normalize_data=True):
    ...
    def _fit(self, train_ep_data, val_ep_data, *args, **kwargs):
        ...
        scores = []
        for data in val_ep_data:
            obs = np.array(data.states)
            acts = np.array(data.actions)
            score = self._predict_scores(obs, acts)
            scores.append(score.max(0))
        self.threshold = min(scores)
