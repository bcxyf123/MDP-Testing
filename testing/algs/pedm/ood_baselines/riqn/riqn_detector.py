...
class RIQN_Detector(Base_Detector):
    def __init__(self, env, model=None, horizon=1, device=None, normalize_data=True, model_kwargs={}, *args, **kwargs):
        ...
        self.model = RIQN_Predictor(input_features=input_dim, **model_kwargs)

    def _predict_scores(self, obs, acts):
        return self.model.predict_episode(obs, sampling_size=4)
    ...
    def _fit(self, train_ep_data, val_ep_data, n_train_epochs=1_000, *args, **kwargs):
        ...
        X_val = np.array([ep.states for ep in val_ep_data])
        scores = []
        for obs in X_val:
            score = self._predict_scores(obs, acts=None).max(0)
            scores.append(score)
