...
import testing.envs.maze.rendering as r
from testing.envs.maze.window import Window
...
class SimpleGridEnv(Env):
    ...
    def __init__(self,     
        ...
        if options is None:
            raise ValueError("You must provide start and goal points in maze!")
    ...
    def compute_log_likelihood(self, transition):
        ...
        x_, y_ = self.to_xy(s_)
        move = (x_ - x, y_ - y)
        REVERSE_MOVES = {v: k for k, v in self.MOVES.items()}
        probs = self.get_transprob(x, y, a)
        
        # if move == (0,0):
        #     for move_ in self.MOVES.values():
        #         target_pos = (x + move[0], y + move[1])
        #         if self.is_in_bounds(*target_pos) or not self.is_free(*target_pos):
        #             move = move_
        #             break
        ...
        if move == (0,0):
            ...
            return np.log(probs[REVERSE_MOVES[move]])
        

    def _chooseAction(self, x, y, action):
        probs = self.get_transprob(x, y, action)
    ...
    def step(self, action: int):
        ...
        if not self.deterministic:
            action = self._chooseAction(row, col, action)
    ...
    def get_reward(self, x: int, y: int) -> float:
        ...
        elif self.steps >= self.max_episode_steps and not self.on_goal():
            return -1.0
        # sparse reward -- relative distance
        else:
            # return -np.sum(np.abs(np.array(self.goal_xy) - np.array(self.agent_xy))) / (self.nrow + self.ncol)
            # return np.random.choice([-0.1, 0], p=[0.9, 0.1])
            return -0.1
    ...
    def get_done(self) -> bool:
        return self.on_goal() or self.steps >= self.max_episode_steps
