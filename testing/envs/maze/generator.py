...
def genMaze(map_size=None, obstacle_locs=None, max_prob=0.9, start_loc=None, goal_loc=None, deterministic=False, max_episode_steps=50, seed=None):
    # in non-approximation methods, it's best to keep the state space the same
    if map_size is None:
        raise ValueError('map_size must be provided')
    if obstacle_locs is None:
        raise ValueError('obstacle_locs must be provided')
    ...
    if deterministic is False:
        if max_prob is None:
            raise ValueError('max_prob must be provided in stochastic environment!')
    if seed is None:
