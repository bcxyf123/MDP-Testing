...
class gridTrainer(object):
    ...
    #     self.env.reset()
        ...
        self.map_id = map_id
        self.env = genEnv(map_id=map_id, max_prob=0.6, start_loc=start_loc, goal_loc=goal_loc, deterministic=False)
        # self.reset = envGen.reset

        num_states = self.env.observation_space.n
        num_actions = 4
        self.learner = QLearner(num_states, num_actions, gamma=0.9)
    ...
    def plot(self, epochs, rewards):
        ...
        plt.savefig(f'save/img/train/map_{self.map_id}_rewards_{epochs}')
