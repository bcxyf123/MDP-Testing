...
def collect_trajectories(env, policy, episodes=10):
    ...
    for i in range(episodes):
        ...
        steps = 0
        # begin token
        transition = [np.array(np.ones_like(obs)*(-1)).reshape(-1, 1), np.array(-1).reshape(-1, 1), 0.0, np.array(5).reshape(-1, 1)]
        ...
        while not (done or truncated):
            ...
            if done or truncated:
                ...
                delta_obs = 5
                # delta_obs = 0
            transition = [np.array(obs).reshape(-1, 1), np.array(action).reshape(-1, 1), reward, np.array(delta_obs).reshape(-1, 1)]
            transitions.append(transition)
            obs = new_obs
...
def collect_transitions(env, policy, num_steps=1000):
    ...
    return np.array(observations).reshape(len(observations), -1), np.array(actions).reshape(len(observations), -1), np.array(delta_obs_list)
...
class TransitionModel(nn.Module):
    def __init__(self, obs_dim, action_dim, num_classes=6, hidden_size=32):
        ...
        self.mean = nn.Linear(hidden_size, obs_dim)
        self.log_std = nn.Linear(hidden_size, obs_dim)
...
def train_transition_model(model, observations, actions, delta_observations, epochs=1_000, batch_size=32, save_dir=None, val_split=0.2):
    ...
    for epoch in range(epochs):
        ...
        # model.eval()
        # with torch.no_grad():
        #     val_loss = sum([-torch.distributions.Normal(model(obs_batch, action_batch)).log_prob(delta_obs_batch).mean() for obs_batch, action_batch, delta_obs_batch in val_loader])
...
def main(cfg: DictConfig):
    ...
    env = genEnv(map_size=(4,8), map_id=0, max_prob=0.9, start_loc=(3,0), goal_loc=(0,7), deterministic=cfg.train.deterministic)
    # # train forward model
    ...
    map_list = range(7)
    # prob_list = np.arange(0.6, 1.0, 0.005)
    # for i, prob in enumerate(prob_list):
    for i, map_id in enumerate(map_list):
        print(f'Training model {i+1}/{len(map_list)}, map_id={map_id}')
        load_dir = os.path.join(cfg.train.load_dir, f'map_0_policy_2000.csv')
        # save_dir = os.path.join(cfg.train.save_dir, f'forward_model_{prob:.3f}.pth')
        save_dir = os.path.join(cfg.train.save_dir, f'forward_model_{map_id}.pth')
        policy = read_csv(load_dir)[:, -1]
        # generate test environment
        env = genEnv(map_size=(4,8), map_id=map_id, max_prob=0.9, start_loc=(3,0), goal_loc=(0,7), deterministic=cfg.train.deterministic)
        # # train forward model
        obs, actions, delta_obs = collect_transitions(env, policy, num_steps=cfg.train.collect_steps)
        # use model ensemble
        # models = [TransitionModel(obs_dim=env.observation_space['obs'].shape[0], action_dim=1) for _ in range(cfg.train.model_num)]
