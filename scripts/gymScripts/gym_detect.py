...
def policy_rollout_fixed(env, policy, max_steps=100):
...
def policy_rollout(env, policy, max_steps=100):
    states, actions, rewards, dones = (
        [],
        [],
        [],
...
def train_detector(env, policy, detector_type, collect_episodes):
    if 'pedm' in detector_type:
        ...
        detector = RIQN_Detector(env.env, horizon=5, normalize_data=False, model_kwargs={'gru_units': 8, 'quantile_embedding_dim': 32, 'num_quantile_sample': 8, 'device': 'cpu'})
...
def detect_powers(test_env, policy, detector, num_trajs):
    ...
    for _ in range(num_trajs):
        ...
        states_1, actions_1, _, _ = policy_rollout_fixed(test_env, policy, max_steps=100)
...
def Parameter_name_list(env_name):
    if 'CartPole' in env_name:
        ...
        param_list = ['LINK_LENGTH_1', 'LINK_MASS_1', 'LINK_MASS_2']
        policy_name = 'ppo_acrobot.zip'
    elif 'MountainCar' in env_name:
        param_list = ['min_position', 'max_position', 'max_speed', 'goal_position', 'goal_velocity', 'force', 
                'gravity', 'start_position', 'start_position_std', 'start_velocity', 'start_velocity_std']
        policy_name = 'MountainCar.zip'
    elif 'LunarLander' in env_name:
        param_list = ['FPS', 'SCALE', 'MAIN_ENGINE_POWER', 'SIDE_ENGINE_POWER', 'INITIAL_RANDOM', 'LANDER_W', 'LANDER_H', 'LEG_AWAY', 'LEG_DOWN', 'VIEWPORT_W', 'VIEWPORT_H']
...
def self_test():
    ...
    detector = train_detector(env_0, policy, detector_type=detector_name, collect_episodes=50)
    print('training finished')
    powers = []

    print(f"Currently using {env_name} {detector_name} {param_name}: ", env_1.context[param_name])
    power = detect_powers(env_1, policy, detector, num_trajs=100)
    powers.append(power)
    print()
...
def single_test():
    ...
    # detector_name = 'riqn'
    # for detector_name in ['gmm', 'gaussian', 'knn', 'forest', ]:
    ...
    # param_name = 'torque_noise_max'
        ...
        # for detector_name in ['pedm', ]:
            ...
            for i in range(0, len(contexts)):
                ...
                print(f"Currently using {env_name} {detector_name} {param_name}: ", env_1.context[param_name])
                print(f"model {i}/{len(contexts)-1}")
                power = detect_powers(env_1, policy, detector, num_trajs=10)
                powers.append(power)
                print()
