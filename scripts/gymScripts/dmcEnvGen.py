...
def generate_FingerEnvs(parameter):
    ...
    f_torsional_list = [0.1, 0.5, 1.0, 1.5, 2.0, 2.5, 5.0, 10.0]

    f_rolling_list = [0.1, 0.5, 1.0, 1.5, 2.0, 2.5, 5.0, 10.0]

    f_tangential_list = [0.1, 0.5, 1.0, 1.5, 2.0, 2.5, 5.0, 10.0]

    timestep_list = [0.001, 0.0025, 0.005, 0.01, 0.025, 0.05, 0.1]

    joint_damping_list = [0.1, 0.5, 1.0, 1.5, 2.0, 2.5, 5.0, 10.0]

    joint_stiffness_list = [0.1, 0.5, 1.0, 1.5, 2.0, 2.5, 5.0, 10.0]

    actuator_strength_list = [0.1, 0.5, 1.0, 1.5, 2.0, 2.5, 5.0, 10.0]

    density_list = [0.1, 0.5, 1.0, 1.5, 2.0, 2.5, 5.0, 10.0]

    viscosity_list = [0.1, 0.5, 1.0, 1.5, 2.0, 2.5, 5.0, 10.0]

    geom_density_list = [0.1, 0.5, 1.0, 1.5, 2.0, 2.5, 5.0, 10.0]

    wind_x_list = [-0.1, -0.05, -0.01, -0.005, -0.001, 0, 0.001, 0.005, 0.01, 0.05, 0.1]

    wind_y_list = [-0.1, -0.05, -0.01, -0.005, -0.001, 0, 0.001, 0.005, 0.01, 0.05, 0.1]

    wind_z_list = [-0.1, -0.05, -0.01, -0.005, -0.001, 0, 0.001, 0.005, 0.01, 0.05, 0.1]

    limb_length_0_list = [0.01, 0.05, 0.1, 0.15, 0.2]

    limb_length_1_list = [0.01, 0.05, 0.1, 0.15, 0.2]


    if parameter == 'gravity':
        contexts_list = gravity_list
    elif parameter == 'friction_torsional':
        contexts_list = f_torsional_list
    elif parameter == 'friction_rolling':
        contexts_list = f_rolling_list
    elif parameter == 'friction_tangential':
        contexts_list = f_tangential_list
    elif parameter == 'timestep':
        contexts_list = timestep_list
    elif parameter == 'joint_damping':
        contexts_list = joint_damping_list
    elif parameter == 'joint_stiffness':
        contexts_list = joint_stiffness_list
    elif parameter == 'actuator_strength':
        contexts_list = actuator_strength_list
    elif parameter == 'density':
        contexts_list = density_list
    elif parameter == 'viscosity':
        contexts_list = viscosity_list
    elif parameter == 'geom_density':
        contexts_list = geom_density_list
    elif parameter == 'wind_x':
        contexts_list = wind_x_list
    elif parameter == 'wind_y':
        contexts_list = wind_y_list
    elif parameter == 'wind_z':
        contexts_list = wind_z_list
    elif parameter == 'limb_length_0':
        contexts_list = limb_length_0_list
    elif parameter == 'limb_length_1':
        contexts_list = limb_length_1_list
    else:
    ...
    for c in contexts_list:
        env_context = CARLDmcFingerEnv.get_default_context().copy()
        env_context[parameter] = c
        env_contexts.append(env_context)

    contexts = {0: CARLDmcFingerEnv.get_default_context()}
    for i, env_context in enumerate(env_contexts):
        contexts[i+1] = env_context

    env = CARLDmcFingerEnv(context_selector=StaticSelector(contexts=contexts))
