...
from scripts.gymScripts.gymEnvGen import generate_AcrobotEnvs
from gym_detect import Parameter_name_list, genEnv
from train_transition_model import CARL_obs_transform
from utils.utils import *


def reward_func(env_name, obs):
    if 'Acrobot' in env_name:
        reward = -1.0 if not bool(-np.cos(obs[0]) - np.cos(obs[1] + obs[0]) > 1.0) else 0.0
    elif 'MountainCar' in env_name:
        reward = -1.0
    return reward


from utils.utils import return_tester
...
def main(cfg: DictConfig):
    # get important parameters from cfg
    env_name = cfg.detect.env_name
    param_name = cfg.detect.param_name

    # load policy
    policy_path = os.path.join(cfg.train.load_dir, f'{env_name}.zip')
    policy = PPO.load(policy_path)
    env, contexts, _ = genEnv(env_name, param_name)
    env.reset()
    
    # create return tester
    tester = gym_return_tester(policy, num_trajs=cfg.detect.collect_trajs)

    reject_region = tester.dist_estimation(cfg.detect.n_samples, env_name, param_name,)
    powers = []
    for i in range(len(contexts)):
        power = tester.power_analysis(reject_region, env_name, param_name, test_id=i)
        powers.append(power)
        env.context_id = i
        print(f'param: {param_name} {env.context[param_name]}, power: {power:.3f}')

    data = {
            'powers': powers
        }
    save_dir = f'tables/{env_name}/{param_name}/value_new.csv'
    df = pd.DataFrame(data)
    df.to_csv(save_dir, index=False)
